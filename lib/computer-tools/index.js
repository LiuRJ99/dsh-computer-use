...
var INPUT_TOOL_NAMES = /* @__PURE__ */ new Set([
  "computer_use_click",
...
function preflightInputTool(ctx, exec) {
  if (!INPUT_TOOL_NAMES.has(exec.name)) return void 0;
  ...
  try {
    switch (exec.name) {
      case "computer_use_click":
        validateClick(exec.arguments);
        break;
      case "computer_use_type_text":
        validateTypeText(exec.arguments);
        break;
      case "computer_use_press_key":
        validatePressKey(exec.arguments);
        break;
      case "computer_use_scroll":
        validateScroll(exec.arguments);
        break;
      case "computer_use_set_value":
        validateSetValue(exec.arguments);
        break;
      case "computer_use_select_text":
        validateSelectText(exec.arguments);
        break;
      case "computer_use_drag":
        validateDrag(exec.arguments);
        break;
      case "computer_use_perform_secondary_action":
        validateSecondary(exec.arguments);
        break;
...
var COMPUTER_USE_SKILL = {
  ...
  whenToUse: "Invoke /computer-use only when the task requires operating a desktop app or browser, reproducing a GUI-only bug, or verifying a UI flow \u2014 rather than files or command output.",
...
function apply(ctx, config = {}) {
  ...
  ctx.on("tools/pre-execute", (exec, next) => {
    ...
    return violation === void 0 ? next() : Promise.resolve({ kind: "deny", reason: violation });
  }, { prepend: true });
